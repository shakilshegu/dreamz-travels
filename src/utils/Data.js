export const tourPackages = [
  {
    id: 1,
    packageName: "Chikmagalur Tour",
    destination: "Chikmagalur",
    tagline: "Mountain Peaks & Coffee Paradise",
    duration: "3 Days & 2 Nights",
    tariff: "₹5,999",
    perHead: true,
    minimumMembers: 4,
    stayName: "Dreamzz Comforts",
    pickupLocation: "Bengaluru/Mysore",
    rating: 4.9,
    reviewCount: 98,
    image: "https://images.unsplash.com/photo-1573674401446-87cae8d4d28e?q=80&w=1074&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
    highlights: [
      "Mullayanagiri Peak Trek",
      "Baba Budangiri Hills",
      "Jhari & Honnamanahalla Falls",
      "Hebbe & Kalhattigiri Waterfalls",
      "Kemmangundi Hill Station",
      "Belur & Halebidu Temples",
      "Yagachi Water Sports"
    ],
    inclusions: [
      "Stay at Dreamzz Comforts",
      "Private car with driver",
      "Breakfast included",
      "All sightseeing as per itinerary"
    ],
    exclusions: [
      "Entry fees to attractions",
      "Meals not mentioned",
      "Personal expenses"
    ],
    itinerary: [
      {
        day: 1,
        title: "Mountain Peaks & Waterfalls Adventure",
        activities: [
          "Pickup from Bengaluru/Mysore",
          "Check-in at Dreamzz Comforts",
          "Mullayanagiri Peak",
          "Baba Budangiri",
          "Jhari Falls",
          "Honnamanahalla Falls",
          "Siri Café"
        ]
      },
      {
        day: 2,
        title: "Waterfalls & Temple Exploration",
        activities: [
          "Breakfast at Dreamzz Comforts",
          "Hebbe Waterfalls",
          "Kalhattigiri Falls",
          "Kemmangundi",
          "Deviramma Temple"
        ]
      },
      {
        day: 3,
        title: "Temples & Water Sports",
        activities: [
          "Breakfast at Dreamzz Comforts",
          "Yagachi Water Sports",
          "Belur Temples",
          "Halebidu Temples",
          "Checkout & Return"
        ]
      }
    ],
    badge: "Trending",
    discount: "15% OFF",
    contact: {
      phone: "6362040932",
      instagram: "@dreamzz_travels"
    }
  },
{
  id: 2,
  packageName: "Coorg Tour",
  destination: "Coorg",
  tagline: "Coffee Land Paradise",
  duration: "3 Days & 2 Nights",
  tariff: "₹5,499",
  perHead: true,
  minimumMembers: 4,
  stayName: "Golden Residency",
  pickupLocation: "Bengaluru/Mysore",
  rating: 4.8,
  reviewCount: 127,
  image: "/Coorg.png",
  highlights: [
    "Raja’s Seat Valley Views",
    "Madikeri Fort & Museum",
    "Abbey Falls",
    "Dubare Elephant Camp",
    "Namdroling Monastery (Golden Temple)",
    "Coffee Plantation Tour",
    "Talacauvery & Bhagamandala",
    "Mandalpatti Scenic Drive"
  ],
  inclusions: [
    "Stay at Golden Residency",
    "Private car with driver",
    "Breakfast included",
    "All sightseeing as per itinerary"
  ],
  exclusions: [
    "Entry fees to attractions",
    "Meals not mentioned",
    "Personal expenses",
    "Jeep costs where applicable",
    "Anything not mentioned in inclusions"
  ],
  itinerary: [
    {
      day: 1,
      title: "Cultural Heritage & Nature Retreat",
      activities: [
        "Pickup from Bengaluru/Mysore",
        "Check-in at Golden Residency",
        "Raja’s Seat",
        "Madikeri Fort & Museum",
        "Madikeri Tomb",
        "Abbey Falls",
        "Return to stay for dinner & overnight"
      ]
    },
    {
      day: 2,
      title: "Adventure & Spiritual Bliss",
      activities: [
        "Breakfast at Golden Residency",
        "Dubare Elephant Camp",
        "Nisargadhama Island",
        "Chiklihole Reservoir",
        "Namdroling Monastery (Golden Temple)",
        "Coffee Plantation Tour",
        "Return to stay for a cozy night"
      ]
    },
    {
      day: 3,
      title: "Sacred Origins & Scenic Views",
      activities: [
        "Breakfast at Golden Residency",
        "Talacauvery",
        "Bhagamandala",
        "Mandalpatti Hilltop",
        "Checkout & return to Bengaluru/Mysore"
      ]
    }
  ],
  badge: "Most Popular",
  discount: "20% OFF",
  contact: {
    phone: "6362040932",
    instagram: "@dreamzz_travels"
  }
},
{
  id: 3,
  packageName: "Chikmagalur Tour",
  destination: "Chikmagalur",
  tagline: "2 Days of Scenic Peaks & Adventure",
  duration: "2 Days & 1 Night",
  tariff: "₹3,999",
  perHead: true,
  minimumMembers: 4,
  stayName: "Dreamzz Comforts",
  pickupLocation: "Bengaluru Majestic (6 AM)",
  rating: 4.7,
  reviewCount: 64,
  image: "/Chikmaglur2Days.png",
  highlights: [
    "Belur & Halebidu Temples",
    "Yagachi Water Sports",
    "Mullayanagiri Peak",
    "Baba Budangiri Hills",
    "Jhari Falls Jeep Ride",
    "Honnamanahalla Falls",
    "Siri Café"
  ],
  inclusions: [
    "Comfortable stay in Chikmagalur",
    "Transportation with pickup and drop-off",
    "Breakfast on Day 2",
    "All sightseeing as per itinerary"
  ],
  exclusions: [
    "Personal expenses",
    "Lunch & Dinner not included",
    "Any item not listed in inclusions"
  ],
  itinerary: [
    {
      day: 1,
      title: "Adventure & Historical Exploration",
      activities: [
        "Pickup from Bengaluru Majestic at 6 AM",
        "Scenic drive to Chikmagalur",
        "Belur Historical Temples",
        "Halebidu Historical Temples",
        "Yagachi Water Sports",
        "Check-in at Dreamzz Comforts"
      ]
    },
    {
      day: 2,
      title: "Scenic Peaks & Nature Retreat",
      activities: [
        "Breakfast at Dreamzz Comforts",
        "Mullayanagiri Peak",
        "Baba Budangiri",
        "Jhari Falls (Jeep Ride)",
        "Honnamanahalla Falls",
        "Siri Café",
        "Departure to Bengaluru Majestic"
      ]
    }
  ],
  badge: "Weekend Special",
  discount: "10% OFF",
  terms: [
    "50% advance payment and balance at pickup",
    "Operates every weekend",
    "Minimum group size may apply",
    "Dreamzz Travels is not responsible for personal loss or injury"
  ],
  contact: {
    phone: "6362040932",
    instagram: "@dreamzz_travels"
  }
},
{
  id: 4,
  packageName: "Chikmagalur Tour",
  destination: "Chikmagalur",
  tagline: "2 Days of Scenic Peaks & Adventure",
  duration: "2 Days & 1 Night",
  tariff: "₹3,999",
  perHead: true,
  minimumMembers: 4,
  stayName: "Dreamzz Comforts",
  pickupLocation: "Bengaluru Majestic (6 AM)",
  rating: 4.7,
  reviewCount: 64,
  image: "https://images.unsplash.com/photo-1739038034755-3dfe96392252?q=80&w=1331&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
  highlights: [
    "Belur & Halebidu Temples",
    "Yagachi Water Sports",
    "Mullayanagiri Peak",
    "Baba Budangiri Hills",
    "Jhari Falls Jeep Ride",
    "Honnamanahalla Falls",
    "Siri Café"
  ],
  inclusions: [
    "Comfortable stay in Chikmagalur",
    "Transportation with pickup and drop-off",
    "Breakfast on Day 2",
    "All sightseeing as per itinerary"
  ],
  exclusions: [
    "Personal expenses",
    "Lunch & Dinner not included",
    "Any item not listed in inclusions"
  ],
  itinerary: [
    {
      day: 1,
      title: "Adventure & Historical Exploration",
      activities: [
        "Pickup from Bengaluru Majestic at 6 AM",
        "Scenic drive to Chikmagalur",
        "Belur Historical Temples",
        "Halebidu Historical Temples",
        "Yagachi Water Sports",
        "Check-in at Dreamzz Comforts"
      ]
    },
    {
      day: 2,
      title: "Scenic Peaks & Nature Retreat",
      activities: [
        "Breakfast at Dreamzz Comforts",
        "Mullayanagiri Peak",
        "Baba Budangiri",
        "Jhari Falls (Jeep Ride)",
        "Honnamanahalla Falls",
        "Siri Café",
        "Departure to Bengaluru Majestic"
      ]
    }
  ],
  badge: "Weekend Special",
  discount: "10% OFF",
  terms: [
    "50% advance payment and balance at pickup",
    "Operates every weekend",
    "Minimum group size may apply",
    "Dreamzz Travels is not responsible for personal loss or injury"
  ],
  contact: {
    phone: "6362040932",
    instagram: "@dreamzz_travels"
  }
},
{
  id: 5,
  packageName: "Coorg & Chikmagalur Tour",
  destination: "Coorg & Chikmagalur",
  tagline: "3 Days of Scenic Peaks, Waterfalls & Adventure",
  duration: "3 Days & 2 Nights",
  tariff: "₹5,999",
  perHead: true,
  minimumMembers: 4,
  stayName: "Golden Residency & Dreamzz Comforts",
  pickupLocation: "Bengaluru (early morning)",
  rating: 4.8,
  reviewCount: 85,
  image: "/Coorg & Chikmagalur.png",
  highlights: [
    "Dubare Elephant Camp",
    "Abbey Falls",
    "Raja’s Seat",
    "Madikeri Fort",
    "Mullayanagiri Peak",
    "Baba Budangiri Hills",
    "Jhari Falls",
    "Honnamanahalla Falls",
    "Siri Café",
    "Belur & Halebidu Temples",
    "Yagachi Water Sports"
  ],
  inclusions: [
    "Accommodation in Coorg & Chikmagalur",
    "Transportation with fuel & driver",
    "Sightseeing as per itinerary"
  ],
  exclusions: [
    "Personal expenses",
    "Entry fees / activities (Jeep rides, water sports, etc.)",
    "Meals not mentioned in itinerary"
  ],
  itinerary: [
    {
      day: 1,
      title: "Coorg Highlights",
      activities: [
        "Pickup from Bengaluru early morning",
        "Dubare Elephant Camp – interact with elephants & enjoy coracle ride",
        "Abbey Falls – picturesque waterfalls amidst greenery",
        "Raja’s Seat – panoramic sunset views",
        "Madikeri Fort – explore historic fort & museum",
        "Overnight stay in Coorg"
      ]
    },
    {
      day: 2,
      title: "Chikmagalur Peaks & Falls",
      activities: [
        "Early morning drive to Chikmagalur (~4 hrs)",
        "Mullayanagiri Peak – highest peak in Karnataka",
        "Baba Budangiri Hills – scenic drives & trekking spots",
        "Jhari Falls – hidden gem waterfall",
        "Honnamanahalla Falls – peaceful waterfall",
        "Evening at Siri Café – coffee-themed café",
        "Overnight stay in Chikmagalur"
      ]
    },
    {
      day: 3,
      title: "Chikmagalur Adventure & Return",
      activities: [
        "Belur & Halebidu Temples – Hoysala architecture (optional based on time)",
        "Yagachi Water Sports – jet ski, banana ride & more (extra cost)",
        "Evening departure back to Bengaluru"
      ]
    }
  ],
  badge: "Popular Combo",
  discount: "15% OFF",
  terms: [
    "50% advance payment and remaining balance at pickup",
    "Operates on weekends",
    "Minimum group size may apply",
    "Dreamzz Travels is not responsible for personal loss or injury"
  ],
  contact: {
    phone: "7899209021 / 6362040932",
    instagram: "@dreamzz_travels"
  }
},
{
  id: 6,
  packageName: "Chikmagalur Tour",
  destination: "Chikmagalur",
  tagline: "3 Days of Mountain Peaks, Waterfalls & Adventure",
  duration: "3 Days & 2 Nights",
  tariff: "₹8,499",
  perHead: true,
  minimumMembers: 2,
  stayName: "Dreamzz Comforts",
  pickupLocation: "Bengaluru/Mysore",
  rating: 4.8,
  reviewCount: 72,
  image: "https://images.unsplash.com/photo-1611995924260-5866680acb0a?q=80&w=880&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
  highlights: [
    "Mullayanagiri Peak",
    "Baba Budangiri Hills",
    "Jhari Falls",
    "Honnamanahalla Falls",
    "Siri Café",
    "Hebbe Waterfalls",
    "Kalhattigiri Falls",
    "Kemmangundi",
    "Deviramma Temple",
    "Yagachi Water Sports",
    "Belur Temples",
    "Halebidu Historical Temples"
  ],
  inclusions: [
    "Stay at Dreamzz Comforts",
    "Private car with driver",
    "Breakfast",
    "All sightseeing as per itinerary"
  ],
  exclusions: [
    "Entry fees to attractions",
    "Meals not mentioned",
    "Personal expenses"
  ],
  itinerary: [
    {
      day: 1,
      title: "Mountain Peaks & Waterfalls Adventure",
      activities: [
        "Pickup from Bengaluru/Mysore and head to Chikmagalur",
        "Check-in at Dreamzz Comforts and freshen up",
        "Mullayanagiri – Trek to the highest peak in Karnataka",
        "Baba Budangiri – Sacred hill with shrine and scenic views",
        "Jhari Falls – Hidden waterfall amidst the forest",
        "Honnamanahalla Falls – Relax and enjoy nature",
        "Siri Café – Local coffee and scenic views",
        "Return to Dreamzz Comforts for overnight stay"
      ]
    },
    {
      day: 2,
      title: "Waterfalls & Temple Exploration",
      activities: [
        "Breakfast at Dreamzz Comforts",
        "Hebbe Waterfalls – Scenic trek",
        "Kalhattigiri Falls – Serene spot surrounded by greenery",
        "Kemmangundi – Explore gardens and panoramic views",
        "Deviramma Temple – Serene hilltop temple",
        "Return to Dreamzz Comforts for overnight stay"
      ]
    },
    {
      day: 3,
      title: "Temples & Water Sports",
      activities: [
        "Breakfast at Dreamzz Comforts",
        "Yagachi Water Sports – Boating, jet skiing, etc.",
        "Belur Temples – Hoysala architecture",
        "Halebidu Historical Temples – Ancient temple complex",
        "Check-out and return to Bengaluru/Mysore"
      ]
    }
  ],
  badge: "Premium Experience",
  discount: "15% OFF",
  terms: [
    "50% advance payment and balance at pickup",
    "Minimum group size 2 members",
    "Dreamzz Travels is not responsible for personal loss or injury"
  ],
  contact: {
    phone: "6362040932",
    instagram: "@dreamzz_travels"
  }
},
{
  id: 7,
  packageName: "Coorg Tour Couple Package",
  destination: "Coorg",
  tagline: "3 Days of Cultural Heritage & Nature Retreat",
  duration: "3 Days & 2 Nights",
  tariff: "₹8,499",
  perHead: true,
  minimumMembers: 2,
  stayName: "Golden Residency",
  pickupLocation: "Bengaluru/Mysore",
  rating: 4.9,
  reviewCount: 58,
  image: "https://images.unsplash.com/photo-1710612198146-77512950a4b7?q=80&w=1170&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
  highlights: [
    "Raja’s Seat",
    "Madikeri Fort & Museum",
    "Madikeri Tomb",
    "Abbey Falls",
    "Dubare Elephant Camp",
    "Nisargadhama",
    "Chiklihole Reservoir",
    "Namdroling Monastery",
    "Coffee Plantation Tour",
    "Talacauvery",
    "Bhagamandala",
    "Mandalpatti"
  ],
  inclusions: [
    "Stay at Golden Residency",
    "Private car with driver",
    "Breakfast",
    "All sightseeing as per itinerary"
  ],
  exclusions: [
    "Entry fees to attractions",
    "Meals not mentioned",
    "Personal expenses",
    "Any Jeep cost for any places",
    "Anything not listed in inclusions"
  ],
  itinerary: [
    {
      day: 1,
      title: "Cultural Heritage & Nature Retreat",
      activities: [
        "Pickup from Bengaluru/Mysore and head to Coorg",
        "Check-in at Golden Residency and freshen up",
        "Raja’s Seat – peaceful garden with valley views",
        "Madikeri Fort & Museum – explore history and culture",
        "Madikeri Tomb – serene historical site",
        "Abbey Falls – cascading waterfalls",
        "Return to Golden Residency for dinner and overnight stay"
      ]
    },
    {
      day: 2,
      title: "Adventure & Spiritual Bliss",
      activities: [
        "Breakfast at Golden Residency",
        "Dubare Elephant Camp – elephant bathing and feeding",
        "Nisargadhama – bamboo groves and deer park",
        "Chiklihole Reservoir – peaceful relaxation",
        "Namdroling Monastery – Tibetan monastery exploration",
        "Coffee Plantation Tour – learn about Coorg coffee",
        "Return to Golden Residency for overnight stay"
      ]
    },
    {
      day: 3,
      title: "Sacred Origins & Scenic Views",
      activities: [
        "Breakfast at Golden Residency",
        "Talacauvery – sacred origin of River Cauvery",
        "Bhagamandala – holy confluence of three rivers",
        "Mandalpatti – breathtaking hilltop views",
        "Check-out and return to Bengaluru/Mysore"
      ]
    }
  ],
  badge: "Couple Special",
  discount: "15% OFF",
  terms: [
    "50% advance payment and balance at pickup",
    "Minimum group size 2 members",
    "Dreamzz Travels is not responsible for personal loss or injury"
  ],
  contact: {
    phone: "6362040932",
    instagram: "@dreamzz_travels"
  }
},
{
  id: 8,
  packageName: "Munnar Tour",
  destination: "Munnar",
  tagline: "3 Days of Scenic Hills, Waterfalls & Tea Gardens",
  duration: "3 Days & 2 Nights",
  tariff: "₹*,999 (Customizable)",
  perHead: true,
  minimumMembers: 2,
  stayName: "(as per availability or customization)",
  pickupLocation: "Kochi",
  rating: 4.7,
  reviewCount: 45,
  image: "https://images.unsplash.com/photo-1629813538702-64c925934e19?q=80&w=687&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
  highlights: [
    "Cheeyappara Waterfalls",
    "Valara Waterfalls",
    "Tea Plantation Views",
    "Mattupetty Dam",
    "Echo Point",
    "Eravikulam National Park",
    "Top Station",
    "Attukad Waterfalls",
    "Pothamedu Viewpoint"
  ],
  inclusions: [
    "Stay as per availability/customization",
    "Private car with driver",
    "Breakfast",
    "Sightseeing as per itinerary"
  ],
  exclusions: [
    "Entry fees to attractions",
    "Meals not mentioned",
    "Personal expenses"
  ],
  itinerary: [
    {
      day: 1,
      title: "Scenic Drive & Waterfalls",
      activities: [
        "Pickup from Kochi and drive to Munnar",
        "Cheeyappara & Valara Waterfalls",
        "Tea Plantation Views",
        "Check-in at Stay",
        "Overnight stay at Munnar"
      ]
    },
    {
      day: 2,
      title: "Munnar Local Circuit",
      activities: [
        "Breakfast at your stay",
        "Mattupetty Dam",
        "Echo Point",
        "Eravikulam National Park",
        "Top Station – Valley & tea garden views",
        "Overnight stay at Munnar"
      ]
    },
    {
      day: 3,
      title: "Waterfalls & Return",
      activities: [
        "Breakfast at your stay",
        "Attukad Waterfalls",
        "Pothamedu Viewpoint",
        "Drive back to Kochi"
      ]
    }
  ],
  badge: "Customizable",
  discount: "Flexible",
  terms: [
    "50% advance payment and balance at pickup",
    "Minimum group size 2 members",
    "Dreamzz Travels is not responsible for personal loss or injury"
  ],
  contact: {
    phone: "6362040932",
    instagram: "@dreamzz_travels"
  }
},
{
  id: 9,
  packageName: "Ooty Tour",
  destination: "Ooty",
  tagline: "3 Days of Scenic Peaks, Tea Gardens & Local Delights",
  duration: "3 Days & 2 Nights",
  tariff: "₹6,499",
  perHead: true,
  minimumMembers: 4,
  stayName: "Mejastic Crown",
  pickupLocation: "Bengaluru / Mysore",
  rating: 4.8,
  reviewCount: 52,
  image: "https://images.unsplash.com/photo-1589136777351-fdc9c9cab193?q=80&w=1170&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
  highlights: [
    "Botanical Gardens",
    "Ooty Lake",
    "Rose Garden",
    "Thread Garden",
    "St. Stephen's Church",
    "Toy Train Ride to Coonoor",
    "Sim’s Park",
    "Dolphin’s Nose",
    "Tea Factory & Tea Museum",
    "Lamb’s Rock",
    "Doddabetta Peak",
    "Wax World Museum",
    "Pykara Waterfalls & Lake",
    "Local Markets"
  ],
  inclusions: [
    "Stay at Drizzle Drop Inn",
    "Private car with driver",
    "Breakfast",
    "All sightseeing as per itinerary"
  ],
  exclusions: [
    "Entry fees to attractions",
    "Meals not mentioned",
    "Personal expenses",
    "Any cost for additional activities",
    "Anything not mentioned in inclusions"
  ],
  itinerary: [
    {
      day: 1,
      title: "Local Sightseeing & Botanical Delights",
      activities: [
        "Pickup from Bengaluru or Mysore and drive to Ooty",
        "Check-in at Drizzle Drop Inn",
        "Botanical Gardens – explore rare plants and flowers",
        "Ooty Lake – boating",
        "Rose Garden – vibrant roses",
        "Thread Garden – handmade plant art",
        "St. Stephen's Church – historical visit",
        "Return to stay for dinner and overnight"
      ]
    },
    {
      day: 2,
      title: "Coonoor Excursion & Tea Culture",
      activities: [
        "Breakfast at stay",
        "Toy Train Ride to Coonoor",
        "Sim’s Park – garden stroll",
        "Dolphin’s Nose – valley views",
        "Tea Factory & Tea Museum – tea sampling",
        "Lamb’s Rock – panoramic views",
        "Return to stay for overnight"
      ]
    },
    {
      day: 3,
      title: "Scenic Peaks & Departure",
      activities: [
        "Breakfast at Drizzle Drop Inn",
        "Doddabetta Peak – highest point visit",
        "Wax World Museum",
        "Pykara Waterfalls",
        "Pykara Lake – boat ride",
        "Local Markets – shopping",
        "Check-out and return to Bengaluru / Mysore"
      ]
    }
  ],
  badge: "Popular Choice",
  discount: "10% OFF",
  terms: [
    "50% advance payment and balance at pickup",
    "Minimum group size 4 members",
    "Dreamzz Travels is not responsible for personal loss or injury"
  ],
  contact: {
    phone: "6362040932",
    instagram: "@dreamzz_travels"
  }
},
{
  id: 10,
  packageName: "Varkala Tour",
  destination: "Varkala",
  tagline: "3 Days of Beaches, Cliffs & Backwaters",
  duration: "3 Days & 2 Nights",
  tariff: "₹*,999 (Customizable)",
  perHead: true,
  minimumMembers: 2,
  stayName: "(as per availability or customization)",
  pickupLocation: "Trivandrum",
  rating: 4.6,
  reviewCount: 38,
  image: "https://images.unsplash.com/photo-1610817201767-793a9130ce07?q=80&w=1062&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
  highlights: [
    "Varkala Cliff & Beach",
    "Janardana Swami Temple",
    "Kappil Beach & Backwaters",
    "Edava Lakes",
    "Optional Ayurveda Therapy",
    "Anchuthengu Fort & Beach"
  ],
  inclusions: [
    "Stay as per availability/customization",
    "Private car with driver",
    "Breakfast",
    "Sightseeing as per itinerary"
  ],
  exclusions: [
    "Entry fees to attractions",
    "Meals not mentioned",
    "Personal expenses"
  ],
  itinerary: [
    {
      day: 1,
      title: "Cliffside Arrival",
      activities: [
        "Pickup from Trivandrum and drive to Varkala",
        "Varkala Cliff & Beach",
        "Janardana Swami Temple",
        "Check-in at Stay",
        "Overnight stay at Varkala"
      ]
    },
    {
      day: 2,
      title: "Beaches & Backwaters",
      activities: [
        "Breakfast at your stay",
        "Kappil Beach & Backwaters",
        "Edava Lakes",
        "Optional Ayurveda Therapy",
        "Overnight stay at Varkala"
      ]
    },
    {
      day: 3,
      title: "History & Return",
      activities: [
        "Breakfast at your stay",
        "Anchuthengu Fort & Beach",
        "Drive back to Trivandrum"
      ]
    }
  ],
  badge: "Customizable",
  discount: "Flexible",
  terms: [
    "50% advance payment and balance at pickup",
    "Minimum group size 2 members",
    "Dreamzz Travels is not responsible for personal loss or injury"
  ],
  contact: {
    phone: "6362040932",
    instagram: "@dreamzz_travels"
  }
},
{
  id: 11,
  packageName: "Wayanad Tour",
  destination: "Wayanad",
  tagline: "3 Days of Nature, Waterfalls & Wildlife",
  duration: "3 Days & 2 Nights",
  tariff: "₹*,499 (Customizable)",
  perHead: true,
  minimumMembers: 2,
  stayName: "(as per availability or customization)",
  pickupLocation: "Kozhikode (Calicut)",
  rating: 4.5,
  reviewCount: 29,
  image: "https://images.unsplash.com/photo-1607054931379-95e37fc6684b?q=80&w=687&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
  highlights: [
    "Lakkidi Viewpoint",
    "Pookode Lake",
    "Edakkal Caves",
    "Soochipara Waterfalls",
    "Meenmutty Falls",
    "Wayanad Wildlife Safari",
    "Banasura Sagar Dam"
  ],
  inclusions: [
    "Stay as per availability/customization",
    "Private car with driver",
    "Breakfast",
    "Sightseeing as per itinerary"
  ],
  exclusions: [
    "Entry fees to attractions",
    "Meals not mentioned",
    "Personal expenses"
  ],
  itinerary: [
    {
      day: 1,
      title: "Hill Views & Lakes",
      activities: [
        "Pickup from Kozhikode",
        "Lakkidi Viewpoint",
        "Pookode Lake",
        "Check-in at Stay",
        "Overnight stay at Wayanad"
      ]
    },
    {
      day: 2,
      title: "Waterfalls & Caves",
      activities: [
        "Breakfast at your stay",
        "Edakkal Caves",
        "Soochipara Waterfalls",
        "Meenmutty Falls",
        "Overnight stay at Wayanad"
      ]
    },
    {
      day: 3,
      title: "Wildlife & Return",
      activities: [
        "Breakfast at your stay",
        "Wayanad Wildlife Safari",
        "Banasura Sagar Dam",
        "Drive back to Kozhikode"
      ]
    }
  ],
  badge: "Customizable",
  discount: "Flexible",
  terms: [
    "50% advance payment and balance at pickup",
    "Minimum group size 2 members",
    "Dreamzz Travels is not responsible for personal loss or injury"
  ],
  contact: {
    phone: "6362040932",
    instagram: "@dreamzz_travels"
  }
},
{
  id: 12,
  packageName: "Dandeli – Gokarna Tour",
  destination: "Dandeli & Gokarna",
  tagline: "3 Days of Adventure, Beaches & Nature",
  duration: "3 Days & 2 Nights",
  tariff: "On Request",
  perHead: true,
  minimumMembers: 4,
  stayName: "River Touch Resort (River View Cottages)",
  pickupLocation: "Hubli (Customizable)",
  rating: 4.6,
  reviewCount: 38,
  image: "https://images.unsplash.com/photo-1693205118032-9382f7267f55?q=80&w=1164&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
  highlights: [
    "River Touch Resort Stay",
    "Boating, Kayaking & Zorbing",
    "Rain Dance & Campfire Fun",
    "Karwar Beach & Warship Museum",
    "Om Beach & Kudle Beach",
    "Syntheri Rocks / Dandeli Wildlife Sanctuary",
    "Optional River Rafting"
  ],
  inclusions: [
    "Stay at River Touch Resort (2 Nights)",
    "Unlimited Buffet Meals (2 Breakfasts, 2 Lunches, 2 Dinners, Evening Snacks with Tea/Coffee)",
    "Water Activities: Boating, Kayaking, River Swimming & Zorbing",
    "Indoor Activities: Rain Dance, Campfire with Music, Carrom & Badminton",
    "Transportation with fuel & driver",
    "Sightseeing as per itinerary"
  ],
  exclusions: [
    "Personal expenses",
    "Entry fees & adventure activity charges not mentioned in inclusions",
    "Meals outside the resort during Gokarna excursion"
  ],
  itinerary: [
    {
      day: 1,
      title: "Resort Adventure in Dandeli",
      activities: [
        "Pickup from Bengaluru / Hubli",
        "Arrival at Dandeli & check-in to River Touch Resort",
        "Unlimited buffet meals",
        "Water Activities: Boating, Kayaking, River Swimming & Zorbing",
        "Indoor Fun: Rain Dance, Night Campfire, Carrom & Badminton",
        "Leisure evening by the riverside",
        "Overnight stay at River Touch Resort"
      ]
    },
    {
      day: 2,
      title: "Day Excursion to Karwar & Gokarna",
      activities: [
        "Early morning departure from Dandeli",
        "Scenic drive via Karwar",
        "Visit Karwar Beach & Warship Museum",
        "Continue to Gokarna",
        "Sunset at Om Beach & Kudle Beach",
        "Return back to Dandeli by night",
        "Dinner & Overnight stay at River Touch Resort"
      ]
    },
    {
      day: 3,
      title: "Dandeli Highlights & Departure",
      activities: [
        "Morning Nature Walk around the resort",
        "Visit Syntheri Rocks / Dandeli Wildlife Sanctuary",
        "Optional River Rafting (on request)",
        "Lunch at Resort",
        "Return journey towards Bengaluru / Hubli"
      ]
    }
  ],
  badge: "Adventure Special",
  discount: "Flexible",
  terms: [
    "25% advance payment required to confirm the booking",
    "Remaining balance to be paid at pickup",
    "Prices valid for a minimum of 4 members",
    "Cancellations within 8 days of trip will incur penalty",
    "Dreamzz Travels is not responsible for delays due to traffic or weather"
  ],
  contact: {
    phone: "6362040932",
    instagram: "@dreamzz_travels"
  }
},
{
  id: 13,
  packageName: "Kodaikanal Tour",
  destination: "Kodaikanal",
  tagline: "3 Days of Nature, Waterfalls & Scenic Views",
  duration: "3 Days & 2 Nights",
  tariff: "₹6499 per head",
  perHead: true,
  minimumMembers: 4,
  stayName: "(as per availability)",
  pickupLocation: "Bengaluru / Madurai",
  rating: 4.7,
  reviewCount: 42,
  image: "https://images.unsplash.com/photo-1593692716621-1e228b0a9224?q=80&w=1169&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
  highlights: [
    "Silver Cascade Falls",
    "Bryant Park",
    "Coaker’s Walk",
    "Kodaikanal Lake",
    "Pillar Rocks",
    "Pine Forest",
    "Guna Caves",
    "Dolphin’s Nose & Moir Point",
    "Fairy Falls & Vattakanal Falls",
    "Chettiar Park"
  ],
  inclusions: [
    "Stay accommodation",
    "Private car with driver",
    "Breakfast",
    "All sightseeing as per itinerary"
  ],
  exclusions: [
    "Entry fees to attractions",
    "Meals not mentioned",
    "Personal expenses",
    "Any cost for additional activities",
    "Anything not mentioned in inclusions"
  ],
  itinerary: [
    {
      day: 1,
      title: "Local Sightseeing & Nature Delights",
      activities: [
        "Pickup from Bengaluru or Madurai and drive to Kodaikanal",
        "Check-in at stay",
        "Silver Cascade Falls",
        "Bryant Park",
        "Coaker’s Walk",
        "Kodaikanal Lake boating",
        "Upper Lake View",
        "Overnight stay at the stay"
      ]
    },
    {
      day: 2,
      title: "Waterfalls, Peaks & Adventure",
      activities: [
        "Breakfast at the stay",
        "Bear Shola Falls",
        "Pillar Rocks",
        "Pine Forest",
        "Guna Caves",
        "Moir Point",
        "Berijam Lake",
        "Overnight stay at the stay"
      ]
    },
    {
      day: 3,
      title: "Relax, Explore & Departure",
      activities: [
        "Breakfast at the stay",
        "Dolphin’s Nose",
        "Vattakanal Falls",
        "Fairy Falls",
        "Chettiar Park",
        "Local Markets shopping",
        "Check-out and return to Bengaluru / Madurai"
      ]
    }
  ],
  badge: "Nature & Adventure",
  discount: "Flexible",
  terms: [
    "50% advance payment recommended",
    "Remaining balance at pickup",
    "Operates every weekend",
    "Minimum group size may apply",
    "Dreamzz Travels is not responsible for personal loss or injury"
  ],
  contact: {
    phone: "6362040932",
    instagram: "@dreamzz_travels"
  }
},
{
  id: 14,
  packageName: "Alleppey Tour",
  destination: "Alleppey",
  tagline: "3 Days of Houseboat & Backwater Bliss",
  duration: "3 Days & 2 Nights",
  tariff: "₹*,499 (Customizable)",
  perHead: true,
  minimumMembers: 2,
  stayName: "Hotel / Houseboat (as per availability or customization)",
  pickupLocation: "Kochi",
  rating: 4.6,
  reviewCount: 28,
  image: "https://plus.unsplash.com/premium_photo-1697729600773-5b039ef17f3b?q=80&w=1170&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
  highlights: [
    "Houseboat Cruise on Vembanad Lake",
    "Sunset Views over Backwaters",
    "Ambalappuzha Krishna Temple",
    "Marari Beach",
    "Pathiramanal Island"
  ],
  inclusions: [
    "Stay at Hotel/Houseboat (as per availability/customization)",
    "Private car with driver",
    "Breakfast",
    "Sightseeing as per itinerary"
  ],
  exclusions: [
    "Entry fees to attractions",
    "Meals not mentioned",
    "Personal expenses"
  ],
  itinerary: [
    {
      day: 1,
      title: "Houseboat & Backwaters Bliss",
      activities: [
        "Pickup from Kochi and drive to Alleppey",
        "Check-in at houseboat/Hotel",
        "Backwater Cruise – Vembanad Lake",
        "Sunset view over serene waters",
        "Overnight stay in houseboat/Hotel"
      ]
    },
    {
      day: 2,
      title: "Beaches & Temples",
      activities: [
        "Breakfast at the stay",
        "Ambalappuzha Krishna Temple",
        "Marari Beach",
        "Overnight stay in Alleppey"
      ]
    },
    {
      day: 3,
      title: "Island & Departure",
      activities: [
        "Breakfast at the stay",
        "Pathiramanal Island – Bird sanctuary",
        "Check-out and return to Kochi"
      ]
    }
  ],
  badge: "Backwater Special",
  discount: "Flexible",
  terms: [
    "25% advance payment recommended",
    "Remaining balance at pickup",
    "Operates every weekend",
    "Minimum group size may apply",
    "Dreamzz Travels is not responsible for personal loss or injury"
  ],
  contact: {
    phone: "6362040932",
    instagram: "@dreamzz_travels"
  }
},
{
  id: 15,
  packageName: "Thekkady Tour",
  destination: "Thekkady",
  tagline: "3 Days of Wildlife & Nature Adventure",
  duration: "3 Days & 2 Nights",
  tariff: "₹*,499 (Customizable)",
  perHead: true,
  minimumMembers: 2,
  stayName: "(as per availability or customization)",
  pickupLocation: "Kochi / Madurai",
  rating: 4.6,
  reviewCount: 22,
  image: "https://plus.unsplash.com/premium_photo-1698430568270-d217ceff35d1?q=80&w=1117&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
  highlights: [
    "Spice Plantation Tour",
    "Periyar Lake Boating",
    "Periyar Wildlife Sanctuary Jeep Safari",
    "Kalaripayattu & Kathakali Show",
    "Mangala Devi Temple",
    "Local Spice Shopping"
  ],
  inclusions: [
    "Stay at (as per availability/customization)",
    "Private car with driver",
    "Breakfast",
    "Sightseeing as per itinerary"
  ],
  exclusions: [
    "Entry fees to attractions",
    "Meals not mentioned",
    "Personal expenses"
  ],
  itinerary: [
    {
      day: 1,
      title: "Arrival & Spice Plantation",
      activities: [
        "Pickup from Kochi/Madurai and drive to Thekkady",
        "Check-in at Stay",
        "Spice Plantation Tour",
        "Periyar Lake Boating",
        "Overnight stay at Thekkady"
      ]
    },
    {
      day: 2,
      title: "Forest & Culture",
      activities: [
        "Breakfast at the stay",
        "Periyar Wildlife Sanctuary Jeep Safari",
        "Evening Kalaripayattu & Kathakali Show",
        "Overnight stay at Thekkady"
      ]
    },
    {
      day: 3,
      title: "Temples & Return",
      activities: [
        "Breakfast at the stay",
        "Mangala Devi Temple",
        "Local spice shopping",
        "Return to Kochi/Madurai"
      ]
    }
  ],
  badge: "Wildlife Special",
  discount: "Flexible",
  terms: [
    "25% advance payment recommended",
    "Remaining balance at pickup",
    "Operates every weekend",
    "Minimum group size may apply",
    "Dreamzz Travels is not responsible for personal loss or injury"
  ],
  contact: {
    phone: "6362040932",
    instagram: "@dreamzz_travels"
  }
},
{
  id: 16,
  packageName: "Manali Tour",
  destination: "Manali",
  tagline: "4 Days of Mountains, Valleys & Adventure",
  duration: "4 Days / 3 Nights",
  tariff: "On Request",
  perHead: false,
  minimumMembers: 4,
  stayName: "Treebo Premium Silverstone Resort (Customizable)",
  pickupLocation: "Delhi (Volvo/AC Bus)",
  rating: 4.7,
  reviewCount: 38,
  image: "https://images.unsplash.com/photo-1593183981460-e9276b5a5587?w=500&auto=format&fit=crop&q=60&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MTV8fFNoaW1sYXxlbnwwfHwwfHx8MA%3D%3D",
  highlights: [
    "Delhi to Manali Overnight Journey",
    "Hadimba Devi Temple & Van Vihar",
    "Solang Valley Adventure & Rohtang Excursion",
    "Kullu Valley, Kasol & Manikaran Sahib",
    "Mall Road & Tibetan Market Shopping"
  ],
  inclusions: [
    "Delhi–Manali–Delhi Volvo/AC bus tickets",
    "3 Nights hotel stay in Manali (as per chosen category)",
    "Private car for sightseeing as per itinerary",
    "Driver charges, fuel & tolls"
  ],
  exclusions: [
    "Meals & beverages not mentioned in inclusions",
    "Entry fees, adventure activities & Rohtang permits/extra vehicle",
    "Personal expenses, shopping & tips",
    "Early check-in / late check-out charges"
  ],
  itinerary: [
    {
      day: 0,
      title: "Delhi ➝ Manali (Overnight Journey)",
      activities: [
        "Evening pickup from Delhi boarding point by Volvo/AC Semi-Sleeper Bus",
        "Overnight journey via scenic highways through Haryana & Himachal Pradesh",
        "Night stay onboard bus"
      ]
    },
    {
      day: 1,
      title: "Arrival in Manali & Local Sightseeing",
      activities: [
        "Morning arrival at Manali bus stand & transfer to hotel",
        "Visit Hadimba Devi Temple surrounded by cedar forest",
        "Stroll through Van Vihar & Riverfront",
        "Evening leisure at Mall Road & Tibetan Market",
        "Overnight stay in Manali"
      ]
    },
    {
      day: 2,
      title: "Solang Valley Adventure / Rohtang Excursion",
      activities: [
        "Drive to Solang Valley – paragliding, ropeway & ATV rides",
        "Excursion to Rohtang Pass (if open & with permit)",
        "Optional stop at Vashisht Hot Springs / Jogini Waterfalls",
        "Evening free at leisure",
        "Overnight stay in Manali"
      ]
    },
    {
      day: 3,
      title: "Kullu Valley & Manikaran",
      activities: [
        "Scenic drive to Kullu Valley – rafting & shawl factories",
        "Visit Kasol (Mini Israel of India)",
        "Explore Manikaran Sahib Gurudwara & Hot Springs",
        "Local shopping & leisure time",
        "Overnight stay in Manali"
      ]
    },
    {
      day: 4,
      title: "Manali ➝ Delhi (Overnight Journey)",
      activities: [
        "Morning at leisure – café hopping or local exploration",
        "Evening departure from Manali by Volvo/AC Semi-Sleeper Bus",
        "Overnight journey to Delhi (arrival next morning)"
      ]
    }
  ],
  badge: "Himalayan Escape",
  discount: "Flexible",
  terms: [
    "25% advance payment required to confirm the booking",
    "Remaining balance due at the time of pickup",
    "Prices valid for a minimum of 4 members",
    "Rohtang Pass access subject to weather & permits",
    "Dreamzz Travels is not responsible for delays due to traffic, weather, or roadblocks"
  ],
  contact: {
    phone: ["7899209021", "6362040932"],
    instagram: "@dreamzz_travels"
  }
},
{
  id: 17,
  packageName: "Shimla Tour",
  destination: "Shimla",
  tagline: "3 Days of Hills, Colonial Charm & Kufri Adventures",
  duration: "3 Days / 2 Nights",
  tariff: "On Request",
  perHead: false,
  minimumMembers: 4,
  stayName: "Customized (Budget / Standard / Deluxe as per choice & availability)",
  pickupLocation: "Delhi (Volvo/AC Bus)",
  rating: 4.6,
  reviewCount: 29,
  image: "https://plus.unsplash.com/premium_photo-1697729628826-ca05ca7f5e8e?q=80&w=1170&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
  highlights: [
    "Delhi to Shimla Overnight Journey",
    "Christ Church & The Ridge",
    "Mall Road & Lakkar Bazaar Shopping",
    "Kufri Excursion with Adventure Activities",
    "Jakhoo Temple Visit"
  ],
  inclusions: [
    "Delhi–Shimla–Delhi Volvo/AC bus tickets",
    "2 Nights accommodation in Shimla (as per chosen category)",
    "Private car for Kufri & local sightseeing",
    "Driver charges, fuel & tolls"
  ],
  exclusions: [
    "Meals not mentioned in inclusions",
    "Entry fees, adventure activities, horse rides, etc.",
    "Personal expenses, shopping & tips",
    "Early check-in / late check-out"
  ],
  itinerary: [
    {
      day: 0,
      title: "Delhi ➝ Shimla (Overnight Journey)",
      activities: [
        "Evening pickup from Delhi by Volvo/AC bus",
        "Overnight journey through Haryana & Himachal roads",
        "Night stay onboard bus"
      ]
    },
    {
      day: 1,
      title: "Shimla Arrival & Local Sightseeing",
      activities: [
        "Morning arrival at Shimla, transfer to hotel & check-in",
        "Visit Christ Church & The Ridge",
        "Explore Mall Road & Lakkar Bazaar",
        "Leisure walk through pine forests & colonial streets",
        "Overnight stay in Shimla"
      ]
    },
    {
      day: 2,
      title: "Kufri Excursion",
      activities: [
        "Full-day excursion to Kufri",
        "Horse riding to Mahasu Peak (optional)",
        "Visit Himalayan Nature Park (Zoo)",
        "Adventure activities at Kufri Fun World",
        "Return to Shimla in evening, free time for shopping & cafés",
        "Overnight stay in Shimla"
      ]
    },
    {
      day: 3,
      title: "Local Exploration & Departure",
      activities: [
        "Morning visit to Jakhoo Temple (Lord Hanuman’s temple)",
        "Free time for last-minute shopping at Mall Road",
        "Evening departure from Shimla by Volvo/AC bus",
        "Overnight journey to Delhi (arrival next morning)"
      ]
    }
  ],
  badge: "Hill Retreat",
  discount: "Flexible",
  terms: [
    "25% advance payment required to confirm the booking",
    "Remaining balance due at the time of pickup",
    "Prices valid for a minimum of 4 members",
    "Kufri activities subject to weather conditions",
    "Dreamzz Travels is not responsible for delays caused by traffic, weather, or roadblocks"
  ],
  contact: {
    phone: ["7899209021", "6362040932"],
    instagram: "@dreamzz_travels"
  }
},
{
  id: 18,
  packageName: "Dharamshala–McLeodganj Tour",
  destination: "Dharamshala & McLeodganj",
  tagline: "4 Days of Scenic Views, Monasteries & Adventure",
  duration: "4 Days / 3 Nights",
  tariff: "On Request",
  perHead: false,
  minimumMembers: 4,
  stayName: "Customized (Budget / Standard / Deluxe as per availability)",
  pickupLocation: "Delhi (Volvo/AC Bus)",
  rating: 4.7,
  reviewCount: 34,
  image: "https://media.istockphoto.com/id/2222578191/photo/namgyal-monastery-centre-for-tibetan-studies-and-arts-dharamshala-himachal-pradesh-india.webp?a=1&b=1&s=612x612&w=0&k=20&c=wescfMmlnP3yyn6uW_JwowUC0UD7AdLvoVFgqrqxaUM=",
  highlights: [
    "Delhi to Dharamshala Overnight Journey",
    "HPCA Cricket Stadium Visit",
    "Norbulingka Institute – Tibetan Art & Culture",
    "Dalai Lama Temple & Bhagsu Waterfall",
    "Triund Trek / Dharamkot Village Exploration",
    "Naddi View Point Sunset"
  ],
  inclusions: [
    "Delhi–Dharamshala–Delhi Volvo/AC bus tickets",
    "3 Nights hotel stay (Dharamshala/McLeodganj as per itinerary)",
    "Private car for Dharamshala & McLeodganj sightseeing",
    "Driver charges, fuel & tolls"
  ],
  exclusions: [
    "Meals not mentioned in inclusions",
    "Entry fees, trekking charges & adventure activities",
    "Personal expenses, shopping & tips",
    "Early check-in / late check-out"
  ],
  itinerary: [
    {
      day: 0,
      title: "Delhi ➝ Dharamshala (Overnight Journey)",
      activities: [
        "Evening pickup from Delhi boarding point by Volvo/AC Semi-Sleeper Bus",
        "Overnight journey through Punjab & Himachal roads",
        "Night stay onboard bus"
      ]
    },
    {
      day: 1,
      title: "Arrival at Dharamshala & Local Exploration",
      activities: [
        "Morning arrival at Dharamshala, transfer to hotel & check-in",
        "Visit HPCA Cricket Stadium – one of the most scenic stadiums in the world",
        "Explore War Memorial & Tea Gardens",
        "Visit Norbulingka Institute – Tibetan art & culture center",
        "Overnight stay in Dharamshala"
      ]
    },
    {
      day: 2,
      title: "McLeodganj Sightseeing",
      activities: [
        "Visit Dalai Lama Temple Complex & Namgyal Monastery",
        "Explore Bhagsu Nag Temple & Waterfall",
        "Visit St. John’s Church in Wilderness",
        "Evening at Naddi View Point for sunset",
        "Leisure at cafés & Tibetan market shopping",
        "Overnight stay in McLeodganj"
      ]
    },
    {
      day: 3,
      title: "Optional Trekking / Village Excursion",
      activities: [
        "Triund Trek (day trek option with packed lunch) – for adventure seekers",
        "Explore Dharamkot Village for peaceful forest views",
        "Evening free for shopping & local strolls",
        "Overnight stay in McLeodganj"
      ]
    },
    {
      day: 4,
      title: "Dharamshala ➝ Delhi (Overnight Journey)",
      activities: [
        "Morning at leisure – last-minute shopping or café visit",
        "Evening departure from Dharamshala by Volvo/AC Semi-Sleeper Bus",
        "Overnight journey to Delhi (arrival next morning)"
      ]
    }
  ],
  badge: "Himalayan Escape",
  discount: "Flexible",
  terms: [
    "25% advance payment required to confirm the booking",
    "Remaining balance due at the time of pickup",
    "Prices valid for a minimum of 4 members",
    "Triund Trek subject to weather & fitness level",
    "Dreamzz Travels is not responsible for delays caused by traffic, weather, or roadblocks"
  ],
  contact: {
    phone: ["7899209021", "6362040932"],
    instagram: "@dreamzz_travels"
  }
},
{
  id: 19,
  packageName: "Mussoorie Tour",
  destination: "Mussoorie",
  tagline: "3 Days of Hills, Waterfalls & Scenic Views",
  duration: "3 Days / 2 Nights",
  tariff: "On Request",
  perHead: false,
  minimumMembers: 4,
  stayName: "Customized (Budget / Standard / Deluxe as per availability)",
  pickupLocation: "Delhi (Volvo/AC Bus)",
  rating: 4.5,
  reviewCount: 26,
  image: "https://plus.unsplash.com/premium_photo-1661954540196-47f72de1a97c?q=80&w=1170&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
  highlights: [
    "Delhi to Mussoorie Overnight Journey",
    "Kempty Falls & Company Garden",
    "Lal Tibba – Highest Point in Mussoorie",
    "George Everest’s House & Mussoorie Lake",
    "Christ Church & Mall Road"
  ],
  inclusions: [
    "Delhi–Mussoorie–Delhi Volvo/AC bus tickets",
    "2 Nights accommodation in Mussoorie (as per chosen category)",
    "Private car for local sightseeing",
    "Driver charges, fuel & tolls"
  ],
  exclusions: [
    "Meals not mentioned in inclusions",
    "Entry fees, boating charges, horse rides, adventure activities",
    "Personal expenses, shopping & tips",
    "Early check-in / late check-out charges"
  ],
  itinerary: [
    {
      day: 0,
      title: "Delhi ➝ Mussoorie (Overnight Journey)",
      activities: [
        "Evening pickup from Delhi boarding point by Volvo/AC Semi-Sleeper Bus",
        "Overnight journey via Dehradun road towards Mussoorie",
        "Night stay onboard bus"
      ]
    },
    {
      day: 1,
      title: "Mussoorie Arrival & Local Sightseeing",
      activities: [
        "Morning arrival at Mussoorie, hotel check-in",
        "Visit Kempty Falls – famous waterfall & picnic spot",
        "Explore Company Garden (botanical & amusement park)",
        "Evening stroll at Mall Road & Camel’s Back Road",
        "Overnight stay in Mussoorie"
      ]
    },
    {
      day: 2,
      title: "Scenic Points & Leisure",
      activities: [
        "Visit Lal Tibba Scenic Point – highest point in Mussoorie",
        "Explore George Everest’s House – panoramic views",
        "Enjoy boating at Mussoorie Lake",
        "Visit Christ Church – one of the oldest churches in the Himalayas",
        "Evening leisure for shopping & cafés at Mall Road",
        "Overnight stay in Mussoorie"
      ]
    },
    {
      day: 3,
      title: "Mussoorie ➝ Delhi",
      activities: [
        "Morning free for leisure / quick shopping",
        "Afternoon departure from Mussoorie by Volvo/AC Semi-Sleeper Bus",
        "Overnight journey to Delhi (arrival next morning)"
      ]
    }
  ],
  badge: "Hill Retreat",
  discount: "Flexible",
  terms: [
    "25% advance payment required to confirm the booking",
    "Remaining balance due at the time of pickup",
    "Prices valid for a minimum of 4 members",
    "Some sightseeing points may close during rains/winters",
    "Dreamzz Travels is not responsible for delays caused by traffic, weather, or roadblocks"
  ],
  contact: {
    phone: ["7899209021", "6362040932"],
    instagram: "@dreamzz_travels"
  }
},
{
  id: 20,
  packageName: "Mussoorie–Rishikesh Tour",
  destination: "Mussoorie & Rishikesh",
  tagline: "5 Days of Hills, Adventure & Spirituality",
  duration: "5 Days / 4 Nights",
  tariff: "On Request",
  perHead: false,
  minimumMembers: 4,
  stayName: "Customized (Budget / Standard / Deluxe as per availability)",
  pickupLocation: "Delhi (Volvo/AC Bus)",
  rating: 4.6,
  reviewCount: 28,
  image: "https://images.unsplash.com/photo-1583143874828-de3d288be51a?w=500&auto=format&fit=crop&q=60&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Nnx8TXVzc29vcmllfGVufDB8fDB8fHww",
  highlights: [
    "Delhi to Mussoorie Overnight Journey",
    "Kempty Falls & Company Garden",
    "Lal Tibba & George Everest’s House",
    "Mussoorie Lake Boating & Christ Church",
    "Rishikesh – Triveni Ghat, Lakshman Jhula & Ram Jhula",
    "Adventure Activities – River Rafting & Yoga Experiences"
  ],
  inclusions: [
    "Delhi–Mussoorie / Rishikesh–Delhi Volvo/AC bus tickets",
    "2 Nights stay in Mussoorie + 2 Nights stay in Rishikesh (as per chosen category)",
    "Private car for sightseeing in Mussoorie & Rishikesh",
    "Driver charges, fuel & tolls"
  ],
  exclusions: [
    "Meals not mentioned in inclusions",
    "Adventure activities (rafting, bungee, zipline) – payable locally",
    "Entry fees at sightseeing points",
    "Personal expenses, shopping & tips",
    "Early check-in / late check-out charges"
  ],
  itinerary: [
    {
      day: 0,
      title: "Delhi ➝ Mussoorie (Overnight Journey)",
      activities: [
        "Evening pickup from Delhi boarding point by Volvo/AC Semi-Sleeper Bus",
        "Overnight journey via Dehradun road towards Mussoorie",
        "Night stay onboard bus"
      ]
    },
    {
      day: 1,
      title: "Mussoorie Arrival & Local Sightseeing",
      activities: [
        "Morning arrival at Mussoorie, hotel check-in",
        "Visit Kempty Falls – Mussoorie’s most famous waterfall",
        "Explore Company Garden with flowers & mini amusement rides",
        "Evening stroll at Mall Road & Camel’s Back Road",
        "Overnight stay in Mussoorie"
      ]
    },
    {
      day: 2,
      title: "Scenic Points of Mussoorie",
      activities: [
        "Visit Lal Tibba Scenic Point – panoramic Himalayan views",
        "Explore George Everest’s House – heritage site with valley views",
        "Enjoy boating at Mussoorie Lake",
        "Visit Christ Church – oldest in the Himalayas",
        "Evening free for shopping & cafés at Mall Road",
        "Overnight stay in Mussoorie"
      ]
    },
    {
      day: 3,
      title: "Mussoorie ➝ Rishikesh (Transfer & Evening Leisure)",
      activities: [
        "Morning checkout & drive towards Rishikesh (approx. 5 hrs)",
        "Hotel check-in at Rishikesh",
        "Evening visit to Triveni Ghat for Ganga Aarti",
        "Overnight stay in Rishikesh"
      ]
    },
    {
      day: 4,
      title: "Rishikesh Adventure & Spirituality",
      activities: [
        "River Rafting on Ganges (optional adventure activity)",
        "Walk across Lakshman Jhula & Ram Jhula suspension bridges",
        "Explore Beatles Ashram (Maharishi Mahesh Yogi Ashram)",
        "Shopping & café hopping at Tapovan area",
        "Evening leisure by the Ganges",
        "Overnight stay in Rishikesh"
      ]
    },
    {
      day: 5,
      title: "Rishikesh ➝ Delhi (Overnight Journey)",
      activities: [
        "Morning free for yoga/meditation or local shopping",
        "Afternoon departure from Rishikesh by Volvo/AC Semi-Sleeper Bus",
        "Overnight journey to Delhi (arrival next morning)"
      ]
    }
  ],
  badge: "Adventure & Spirituality",
  discount: "Flexible",
  terms: [
    "25% advance payment required to confirm the booking",
    "Remaining balance due at the time of pickup",
    "Prices valid for a minimum of 4 members",
    "River rafting & adventure activities subject to weather & safety conditions",
    "Dreamzz Travels is not responsible for delays due to traffic, weather, or roadblocks"
  ],
  contact: {
    phone: ["7899209021", "6362040932"],
    instagram: "@dreamzz_travels"
  }
},
{
  id: 21,
  packageName: "Pondicherry Tour",
  destination: "Pondicherry",
  tagline: "3 Days of French Charm, Beaches & Spirituality",
  duration: "3 Days / 2 Nights",
  tariff: "On Request",
  perHead: false,
  minimumMembers: 4,
  stayName: "Customized (Budget / Standard / Deluxe as per availability)",
  pickupLocation: "Bengaluru (Private Cab / Tempo Traveller – as per group size)",
  rating: 4.5,
  reviewCount: 22,
  image: "https://images.unsplash.com/photo-1598677833732-7857ae95afce?w=500&auto=format&fit=crop&q=60&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Nnx8UG9uZGljaGVycnl8ZW58MHx8MHx8fDA%3D",
  highlights: [
    "Bengaluru to Pondicherry Scenic Drive",
    "Rock Beach & Promenade Beach",
    "Sri Aurobindo Ashram & Auroville (Matrimandir outer view)",
    "Paradise Beach & Serenity Beach",
    "French Quarter – White Town Café Hopping",
    "Local Markets & French War Memorial"
  ],
  inclusions: [
    "Pickup & drop from Bengaluru by private cab/tempo traveller",
    "2 Nights hotel stay in Pondicherry (as per chosen category)",
    "All local sightseeing in Pondicherry as per itinerary",
    "Driver charges, fuel & tolls"
  ],
  exclusions: [
    "Meals not mentioned in inclusions",
    "Entry fees at monuments & activities (boating, Auroville entry passes)",
    "Personal expenses, shopping & tips",
    "Early check-in / late check-out charges"
  ],
  itinerary: [
    {
      day: 1,
      title: "Bengaluru ➝ Pondicherry (Transfer & Evening Leisure)",
      activities: [
        "Morning pickup from Bengaluru & scenic drive to Pondicherry (approx. 6–7 hrs)",
        "Hotel check-in at Pondicherry",
        "Evening visit to Rock Beach / Promenade Beach",
        "Leisure stroll at Goubert Avenue & local markets",
        "Overnight stay in Pondicherry"
      ]
    },
    {
      day: 2,
      title: "Pondicherry Sightseeing",
      activities: [
        "Visit Sri Aurobindo Ashram – spiritual center",
        "Explore Auroville & Matrimandir (outer view)",
        "Relax at Paradise Beach / Serenity Beach",
        "Visit French War Memorial & Basilica of the Sacred Heart of Jesus",
        "Evening café hopping in White Town (French Quarter)",
        "Overnight stay in Pondicherry"
      ]
    },
    {
      day: 3,
      title: "Local Exploration & Return to Bengaluru",
      activities: [
        "Visit Immaculate Conception Cathedral",
        "Morning leisure at Promenade / local shopping",
        "Afternoon checkout & drive back to Bengaluru",
        "Evening arrival at Bengaluru – trip concludes"
      ]
    }
  ],
  badge: "Beach & Culture",
  discount: "Flexible",
  terms: [
    "25% advance payment required to confirm the booking",
    "Remaining balance due at the time of pickup",
    "Prices valid for a minimum of 4 members",
    "Auroville Matrimandir inside visit requires prior permission (not included)",
    "Dreamzz Travels is not responsible for delays due to traffic, weather, or roadblocks"
  ],
  contact: {
    phone: ["7899209021", "6362040932"],
    instagram: "@dreamzz_travels"
  }
}



];